...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return [u0, x4, x3, x2, x1]  


class EncoderPosEmb(nn.Module):
    def __init__(self, n_channels=3):
        super().__init__()
        self.inc = ConvBlock(n_channels, 64)
        self.down1c = nn.Sequential(nn.MaxPool2d(2), ConvBlock(64, 64))
...
class MultiTaskUnetDVH(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, n_classes_inter=1):
        super().__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.n_classes_inter = n_classes_inter

        self.encoder = Encoder(n_channels)
        self.decoder_seg = Decoder(n_classes)     # 表格线分割
        self.decoder_inter = Decoder(n_classes_inter) # 交点检测

    def forward(self, x):
        features = self.encoder(x)
        mask_seg = self.decoder_seg(features)
        mask_inter = self.decoder_inter(features)
        return mask_seg, mask_inter


class MultiTaskUnetDVHPosEmb(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, n_classes_inter=1):
        super().__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        ...
        self.encoder = EncoderPosEmb(n_channels)
        self.decoder_seg = Decoder(n_classes)
        self.decoder_inter = Decoder(n_classes_inter)
