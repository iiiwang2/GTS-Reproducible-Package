...
TRANSPOSE_FLIP_LEFT_RIGHT = getattr(_TRANSPOSE, "FLIP_LEFT_RIGHT")
...
class DataGenerator(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        annotation_path = self.annotation_lines[index].split(';')[1].strip().replace('\n', '')
    ...
    def get_random_data(self, image, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5, random=True):
        ...
        if not random:
            ...
            #   将图像多余的部分加上灰条
            #---------------------------------#
            image       = image.resize((nw,nh), RESAMPLE_BICUBIC)
        ...
        #------------------------------------------#
        flip = self.rand()<.5
        if flip: image = image.transpose(TRANSPOSE_FLIP_LEFT_RIGHT)
